...
async function generateProjectPollinations(prompt: string): Promise<string> {
...
export async function generateProject(
  ...
  // All OpenRouter models failed — try Pollinations as free final fallback
  onStatus?.("🔄 Trying backup generator");
  logger.warn({ failures }, "All OpenRouter models failed — attempting Pollinations fallback");
  try {
    const raw = await generateProjectPollinations(prompt);
    if (raw && raw.trim().length > 10) {
      ...
      if (parsed && Array.isArray(parsed.files) && parsed.files.length > 0) {
        parsed.files = parsed.files
          .filter((f) => f && typeof f.path === "string" && typeof f.content === "string")
          .map((f) => ({
            path: f.path.replace(/\\/g, "/").replace(/^\/+/, "").replace(/[<>:"|?*\x00-\x1f]/g, "").replace(/\.{2,}/g, ".").replace(/\/\.+\//g, "/").replace(/\/+/g, "/").replace(/^\.+\//, "").slice(0, 255),
            content: typeof f.content === "string" ? f.content : JSON.stringify(f.content, null, 2),
          }))
          .filter((f) => f.path.length > 0 && f.content.length > 0 && !f.path.startsWith("."));
        parsed.name = parsed.name || "nova-project";
        parsed.description = parsed.description || `A ${parsed.type || "web"} project generated by Nova AI`;
        parsed.type = (["static", "react", "nodejs", "fullstack"] as ProjectType[]).includes(parsed.type) ? parsed.type : "static";
        parsed.deploymentTip = parsed.deploymentTip || "Deploy on Netlify for free: app.netlify.com/drop";
        if (parsed.files.length > 0) {
          logger.info({ name: parsed.name, type: parsed.type, files: parsed.files.length }, "Project generated via Pollinations fallback");
          return parsed;
        }
      }
    }
  } catch (err: any) {
    logger.warn({ err: err?.message }, "Pollinations fallback also failed");
    failures.push(`pollinations: ${err?.message?.slice(0, 60) || "failed"}`);
  }
